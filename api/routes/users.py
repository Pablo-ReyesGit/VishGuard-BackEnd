...
from schemas.token import Message
...
def read_users(session: SessionDep, skip: int = 0, limit: int = 100) -> Any:
    ...
    statement = select(User).order_by(col(User.id).desc()).offset(skip).limit(limit)
...
def create_user(*, session: SessionDep, user_in: UserCreate) -> Any:
    """
    Create new user. (envío de correo de bienvenida deshabilitado por ahora)
    """
